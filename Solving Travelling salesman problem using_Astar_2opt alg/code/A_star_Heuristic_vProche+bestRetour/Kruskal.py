    ...
    def getMSTCost(self, sol, source):
        if not isinstance(sol, Solution):
            ...
            for i in range(sol.g.N):
                if sol.g.get_edge(i,v).cost <= min_dist and i not in sol.visited:
                    min_dist = sol.g.get_edge(i,v).cost
                    prox_v=i
            print('distance min de retour: ',min_dist)
